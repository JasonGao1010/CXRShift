...
def split_patient_ids(
    ...
    *,
    ...
    """Stratify by challenge target while assigning each patientId once."""
...
def normalize_to_uint8(array: Any) -> Any:
    """Map one DICOM pixel array to 8-bit grayscale without global statistics."""
    import numpy as np
...
def convert_dicom_to_png(source: Path, destination: Path) -> None:
    """Decode one DICOM image and write the documented grayscale representation."""
    import pydicom
...
def build_image_index(images_dir: Path) -> dict[str, Path]:
    """Index candidate images by stem and reject ambiguous patientId mappings."""
