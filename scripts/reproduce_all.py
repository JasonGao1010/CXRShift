...
def analyze_stage(args: argparse.Namespace) -> None:
    work = args.work_dir
    run(command("scripts/audit_integrity.py", "--dataset", work / "data/kermany_grouped_seed42",
                "--dataset", work / "data/rsna_binary", "--results-dir", work / "results",
                "--output", work / "audit/integrity.json"), dry_run=args.dry_run)
    run(command("scripts/summarize_results.py", "--results-dir", work / "results",
