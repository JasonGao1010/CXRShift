...
def audit_evaluations(results_dir: Path, tolerance: float = 2e-7) -> dict[str, Any]:
    """Recompute metrics for every evaluation JSON that references predictions."""
    checked = 0
    mismatches: list[dict[str, Any]] = []
    missing_predictions: list[dict[str, str]] = []
    # Canonical rebuilds use ``__evaluation``. The second pattern preserves
    # compatibility with the original experiment records.
    evaluation_paths = sorted(
        set(results_dir.glob("*__evaluation.json"))
        | set(results_dir.glob("*eval_*.json"))
    )
...
def main() -> int:
    ...
    datasets = args.dataset or [
        Path("data/processed/kermany_grouped_seed42"),
        Path("data/processed/rsna_binary"),
    ]
    payload: dict[str, Any] = {
        "schema_version": 2,
        "datasets": [
