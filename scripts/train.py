...
def create_classifier_model(
    ...
    """Create a classifier while replacing only the architecture-specific head."""
...
def build_domain_balanced_sampler(
    ...
    """Give each source equal expected mass regardless of its image count."""
    if not prefixes:
        return None, {}
...
def run_epoch(
    *,
    ...
    """Run one train or evaluation epoch and aggregate sample-weighted metrics."""
...
def train(settings: TrainSettings, json_output: Path | None) -> dict[str, Any]:
    """Train one fully specified run and persist its configuration and checkpoints."""
